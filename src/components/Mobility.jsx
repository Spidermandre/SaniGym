...
import { S, T1, T2, T3, T4, INK } from "../styles.js";
...
export default function Mobility() {
  ...
  const palette = { pre: ["#ffb347", "#ff7a00"], post: ["#c084fc", "#7c3aed"], rest: ["#4ff5e2", "#0abab5"] };
  const tips = {
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display: "flex", gap: 4, padding: 4, borderRadius: 16, background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.10)", marginBottom: 18 }}>
        {tabs.map((t) => (
          <button key={t.key} onClick={() => { setTab(t.key); setChecked({}); }} style={{
            ...
            border: tab === t.key ? "1px solid rgba(255,255,255,.18)" : "1px solid transparent",
            background: tab === t.key ? "rgba(255,255,255,.14)" : "transparent",
            boxShadow: tab === t.key ? "0 2px 10px rgba(0,0,0,.4), inset 0 1px 0 rgba(255,255,255,.16)" : "none",
            color: tab === t.key ? T1 : T3, fontFamily: "system-ui,sans-serif",
          }}>{t.emoji} {t.label}</button>
        ))}
      </div>

      <div style={{ ...S.glass, padding: "13px 16px", marginBottom: 16, background: `${c0}1f`, borderColor: `${c0}55` }}>
        ...
        <div style={{ background: "rgba(255,255,255,.10)", borderRadius: 99, height: 4, marginTop: 10, overflow: "hidden" }}>
          ...
          <div key={k} style={{ ...S.glass, padding: "14px 16px", marginBottom: 10, opacity: done ? .55 : 1, transition: "opacity .3s" }}>
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
              <button onClick={() => toggle(k)} aria-label={done ? "Segna da fare" : "Segna fatto"} style={{
                ...
                border: `2px solid ${done ? c0 : "rgba(255,255,255,.22)"}`,
                background: done ? g : "transparent",
                display: "flex", alignItems: "center", justifyContent: "center",
              }}>
                {done && <span style={{ fontSize: 16, color: INK, fontWeight: 800 }}>✓</span>}
              </button>
